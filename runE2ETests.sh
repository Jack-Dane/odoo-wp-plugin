...
docker exec $dockerPHPContainerId /bin/sh -c 'cd /usr/src/odoo_conn; composer install --no-cache'

echo "Running Endpoint Tests"
docker exec $dockerPHPContainerId /bin/sh -c 'cd /usr/src/odoo_conn; vendor/bin/phpunit tests/end_to_end_tests/tests/endpoint_tests'
